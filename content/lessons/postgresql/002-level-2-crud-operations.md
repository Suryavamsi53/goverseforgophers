# Level 2 - CRUD Operations

## 1. Learning Objectives
* **What you'll learn**: Master the core mechanics of Level 2 - CRUD Operations in PostgreSQL.
* **Why it matters**: Crucial for building scalable, high-performance, and robust backend systems.

---

## 2. Topics Covered
* Database Operations
* Table Operations
* Column Operations
* Insert Operations (RETURNING)
* Read Operations
* Update
* Delete


---

## 3. Production Usage Scenarios (Real-world Examples)
For each concept, here is how we use it in a real production environment at scale:

### Scenario: Database Operations
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Table Operations
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Column Operations
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Insert Operations (RETURNING)
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Read Operations
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Update
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)

### Scenario: Delete
**Context**: Imagine you are building a highly concurrent backend system.
**The Problem**: (Define the engineering challenge)
**The PostgreSQL Solution**: (How Level 2 - CRUD Operations solves it effectively in production)



---

## 4. Code & Query Implementation

### 🔹 Basic Implementation
```sql
-- Standard Insert
INSERT INTO users (email, name) VALUES ('test@test.com', 'John');
```

### 🔹 Advanced / Optimized Implementation
```sql
-- Bulk Insert with RETURNING (Avoids a 2nd SELECT query)
INSERT INTO logs (level, message)
VALUES 
  ('INFO', 'Server started'),
  ('ERROR', 'DB disconnected')
RETURNING id, created_at;

-- Update and return the old data for queue processing
UPDATE orders SET status = 'processed'
WHERE status = 'pending'
RETURNING id, customer_id;
```

---

## 5. Internals & Under the Hood
* **How PostgreSQL engine processes this**: (Parser -> Planner -> Executor)
* **Storage impact**: WAL logs, Heap, and TOAST considerations.

---

## 6. Performance & Benchmarking
* **EXPLAIN ANALYZE impacts**
* **Memory vs Disk Trade-offs**

---

## 7. Best Practices & Common Mistakes
* ✅ **Do**: Follow standard PostgreSQL conventions.
* ❌ **Don't**: Ignore index overhead or transaction locking.
* ⚠️ **Common Mistake**: N+1 queries, missing indexes on foreign keys.

---

## 8. Interview Questions
1. **Beginner**: Explain Level 2 - CRUD Operations to a junior dev.
2. **Intermediate**: How would you optimize queries involving Level 2 - CRUD Operations?
3. **Expert**: Describe the low-level locking and memory behavior of Level 2 - CRUD Operations in high-concurrency environments.
